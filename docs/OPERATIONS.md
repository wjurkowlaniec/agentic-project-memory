...
Adapters support Codex, Hermes, Devin, Windsurf capture, Claude Code, and current Antigravity SQLite conversations. Antigravity checks both `~/.gemini/antigravity/` and `~/.gemini/antigravity-ide/`, requires an exact project URI in trajectory metadata, and skips opaque legacy `.pb` files. Commands are explicit and local; there is no daemon or cloud service. Benchmarks use synthetic fixtures and aggregate metrics, so results vary with model, hardware, residency, context, and endpoint availability.
