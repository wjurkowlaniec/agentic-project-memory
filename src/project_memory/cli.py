...
def _parser() -> argparse.ArgumentParser:
    ...
    commands = subs.add_parser("commands"); commands.add_argument("--limit", type=int, default=50); commands.add_argument("--root")
    return parser
    ...
    try:
        ...
        try:
            if args.command == "commands":
                for row in service.list_commands(args.limit):
            ...
            if args.command == "sync":
                ...
                print(f"Synced {result.get('messages', 0)} message(s) in {result.get('sessions', 0)} session(s); pending extraction: {result.get('pending', 0)}; project command event(s): {result.get('commands', 0)}.")
                return 0
